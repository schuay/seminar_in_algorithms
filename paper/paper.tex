...
As in Section \ref{sec:sundell}, deletion flags are packed into each node's pointers. However,
% Deletion flag only on next[0]!

%  ``Entirely limited by logical deletion in DeleteMin''

 % Mention the BoundOffset parameter (p17), and benching using Fraser-based impls.
...
in order to achieve improved performance \cite{linden2013skiplist}. % TODO: Cites for this entire section.
...
in \cite{wimmer2013data}. An open-source implementation is available in the task-scheduling system
\emph{Pheet}\footnote{\url{http://pheet.org}}.

The \citeauthor{wimmer2013data} priority queue was evaluated using a label-correcting variant
of Dijkstra's shortest path algorithm \cite{}. Their model creates a task for each node
